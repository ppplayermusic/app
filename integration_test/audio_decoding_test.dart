...
void main() {
  ...
  Future<void> testFormat(String extension) async {
    ...
    expect(
      completed,
      isTrue,
      reason: 'Player did not complete playback for $extension',
    );
  }
  ...
  testWidgets('Error regression: inaccessible/corrupt files emit error', (
    tester,
  ) async {
    final player = Player();
    ...
    expect(
      hasError,
      isTrue,
      reason: 'Player should emit error for corrupt/inaccessible file',
    );
  });
}
