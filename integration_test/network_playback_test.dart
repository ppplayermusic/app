...
void main() {
  ...
  testWidgets('Network HLS Playback Smoke Test', (WidgetTester tester) async {
    ...
    print(
      'State after 10s: playing=${player.state.playing}, pos=${player.state.position}, width=${player.state.width}, height=${player.state.height}',
    );
    expect(
      player.state.playing,
      isTrue,
      reason: 'Player failed to enter playing state',
    );
    ...
    expect(
      hasError,
      isFalse,
      reason: 'Player emitted an error during network playback',
    );
  });
}
