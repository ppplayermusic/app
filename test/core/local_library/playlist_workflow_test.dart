...
void main() {
  ...
  test('Duplicate operations: import, remove, reorder, export', () async {
    ...
    await db
        ...
        .insert(
          LocalFilesCompanion.insert(
            ...
            mechanism: ls.TrackSourceType.absolutePath.name,
          ),
        );
    await db
        .into(db.tracks)
    ...
    await (db.update(db.playlistTracks)
          ..where((t) => t.id.equals(int.parse(tracks[1].queueItemId!))))
        .write(const PlaylistTracksCompanion(position: drift.Value(0)));
    await (db.update(db.playlistTracks)
          ..where((t) => t.id.equals(int.parse(tracks[0].queueItemId!))))
        .write(const PlaylistTracksCompanion(position: drift.Value(1)));
    ...
    final result = M3uHandler.generate(
      tracks,
      exportDestinationDir: destDir.path,
    );

    expect(result.content, contains('B.mp3'));
    expect(result.content, contains('A.mp3'));
    expect(
      result.content.indexOf('B.mp3'),
      lessThan(result.content.indexOf('A.mp3')),
    );
    ...
    await db
        ...
        .insert(
          LocalFilesCompanion.insert(
            ...
            deduplicationKey: 'C',
            lastScannedAt: DateTime.now(),
            mechanism: ls.TrackSourceType.androidContentUri.name,
            importRootLocator: const drift.Value('/storage/emulated/0/Music'),
          ),
        );
    await db
        .into(db.tracks)
