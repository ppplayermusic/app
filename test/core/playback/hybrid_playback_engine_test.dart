...
class MockPlaybackEngine implements PlaybackController {
  ...
  Future<void> stop() async {
    callLog.add('stop()');
    updateStatus(_currentStatus.copyWith(state: PlaybackState.idle, track: null));
  }
...
void main() {
  ...
  group('HybridPlaybackEngine', () {
    ...
    setUp(() {
      ...
      // Initial track
      engine.prepare(const PlaybackTrack(id: 'test_vid', title: 'Test', artist: 'Test'));
    });
    ...
    test('Rapid minimize/reopen takes latest transition', () async {
      ...
      engine.play(const PlaybackTrack(id: 'test_vid', title: 'Test', artist: 'Test'));
    ...
    test('Track changes during transfer', () async {
      engine.play(const PlaybackTrack(id: 'test_vid1', title: 'Test1', artist: 'Test'));
      
      background.playCompleter = Completer<void>();
      PipHandler.simulateActivityStopped();
      
      // Change track while transferring
      engine.play(const PlaybackTrack(id: 'test_vid2', title: 'Test2', artist: 'Test'));
      background.playCompleter!.complete();
    ...
    test('Track ends during transfer deduplicates exactly once', () async {
      ...
      foreground._eventController.add(const PlaybackEvent(type: PlaybackEventType.trackEnded, track: PlaybackTrack(id: 'test_vid', title: 'Test', artist: 'Test')));
      
      // Destination might also fire trackEnded if it was near the end
      background._eventController.add(const PlaybackEvent(type: PlaybackEventType.trackEnded, track: PlaybackTrack(id: 'test_vid', title: 'Test', artist: 'Test')));
    ...
    test('Source pause is acknowledged before destination playback starts', () async {
      engine.play(const PlaybackTrack(id: 'test_vid', title: 'Test', artist: 'Test'));
    ...
    test('Destination emits playing after handoff timeout triggers containment', () async {
      engine.play(const PlaybackTrack(id: 'test_vid', title: 'Test', artist: 'Test'));
      ...
      foreground._eventController.add(const PlaybackEvent(type: PlaybackEventType.trackEnded));
      
      // Let's fire a late playing status from foreground
      foreground.updateStatus(const PlaybackStatus(state: PlaybackState.playing));
      await Future.delayed(const Duration(milliseconds: 10));
