...
void main() {
  test('Vulnerability 1: Old event arriving after new open() completes is accepted', () async {
    final engine = TestPlaybackEngine();
    await engine.playWithDelay('http://source.a', const Duration(milliseconds: 10));
    
    final playB = engine.playWithDelay('http://source.b', const Duration(milliseconds: 50));
    ...
    expect(isStale, isFalse, 
        reason: 'The event is accepted because the engine cannot distinguish origin after unlocking');
  });

  test('Race Condition: Two opens overlapping and completing out of order', () async {
    final engine = TestPlaybackEngine();
    
    // Start A (takes 100ms)
    final playA = engine.playWithDelay('http://source.a', const Duration(milliseconds: 100));
    
    // Start B (takes 20ms) - supersedes A immediately in _playGeneration
    final playB = engine.playWithDelay('http://source.b', const Duration(milliseconds: 20));
  ...
  test('Vulnerability 2: Valid initialization events during open() are blocked, leaving state incomplete', () async {
    final engine = TestPlaybackEngine();
    
    // playWithDelay fires a valid event halfway through the open() process.
    await engine.playWithDelay('http://source.a', const Duration(milliseconds: 50));
    
    expect(engine.stateIncomplete, isTrue, 
        reason: 'The initialization events were blocked because _mediaKitReadyGeneration was not set yet');
  });
}
