...
void main() {
  ...
  Track createTrack(
    String id, {
    QueueItemOrigin origin = QueueItemOrigin.context,
  }) {
    return Track(
      spotifyId: id,
      name: 'Track $id',
  ...
  test(
    'Autoplay fetches recommendations when approaching end of queue',
    () async {
      final notifier = container.read(playerProvider.notifier);
  ...
  test(
    'Autoplay deduplicates tracks already in queue or seen in session',
    () async {
      final notifier = container.read(playerProvider.notifier);
  ...
  test(
    'User tracks are placed before autoplay tracks when adding to queue',
    () async {
      final notifier = container.read(playerProvider.notifier);
  ...
  test('Autoplay uses dominant artist as fallback context', () async {
    ...
    final t1 = Track(
      ...
      artistId: 'dom_artist',
      artistName: 'Dom',
      queueOrigin: QueueItemOrigin.context,
    );
    final t2 = Track(
      spotifyId: '2',
      name: 'T2',
    ...
    await notifier.playTrack(
      t1,
      queue: [t1, t2, t3],
    ); // No explicit contextArtistId provided
    await Future.delayed(Duration.zero);
