...
void main() {
  test('V11 to V12 migration creates stream_playlists and stream_channels', () async {
    ...
    final db = AppDatabase.forTesting(
      NativeDatabase.opened(sqliteDb),
    );
    ...
    final playlistTableInfo = sqliteDb.select("PRAGMA table_info('stream_playlists');");
    expect(playlistTableInfo, isNotEmpty);
    final playlistCols = playlistTableInfo.map((row) => row['name'] as String).toList();
    expect(playlistCols, containsAll(['id', 'title', 'source_kind', 'source_uri', 'last_refreshed']));

    // Verify stream_channels table exists
    final channelTableInfo = sqliteDb.select("PRAGMA table_info('stream_channels');");
    expect(channelTableInfo, isNotEmpty);
    final channelCols = channelTableInfo.map((row) => row['name'] as String).toList();
    expect(channelCols, containsAll(['id', 'playlist_id', 'title', 'stream_url', 'tvg_id', 'logo', 'group_title', 'position']));
