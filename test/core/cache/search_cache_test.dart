...
void main() {
  ...
  test('watchSearch uses SWR and deduplicates identical canonical queries', () async {
    when(() => mockClient.search(any(), limit: any(named: 'limit'))).thenAnswer((_) async {
      return {'mock': 'fresh_data'};
    });

    final query1 = 'The Beatles';
    final query2 = 'the beatles '; // Different display case/whitespace, same canonical identity
    ...
    await sub1.cancel();
    await sub2.cancel();
    await Future.delayed(const Duration(milliseconds: 100)); // wait for cancellation to settle
  });
  ...
  test('watchSearch emits stale data then fresh data', () async {
    ...
    await db.into(db.catalogCacheEntries).insert(
      CatalogCacheEntriesCompanion.insert(
        key: CacheKeyBuilder.search(normalized, 'track,artist,album,playlist', 'US', limit: 20, offset: 0),
