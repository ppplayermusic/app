...
void main() {
  ...
  testWidgets('PlayerScreen keeps video slot mounted during queue transitions and breakpoints', (WidgetTester tester) async {
    final track = Track(
      youtubeVideoId: 'test',
      spotifyId: 'test',
      name: 'Test',
      ...
      ProviderScope(
        overrides: [
          ...
          playbackStatusProvider.overrideWith(
            (ref) => Stream.value(const PlaybackStatus(state: PlaybackState.playing, hasVideo: true)),
          ),
    ...
    final videoSlotFinder = find.byWidgetPredicate((widget) =>
    ...
    ProviderScope.containerOf(context).read(settingsProvider.notifier).setPlayerView(PlayerView.video);
    for (var i = 0; i < 10; i++) {
      await tester.pump(const Duration(milliseconds: 50));
    }
    expect(find.text('QUEUE'), findsOneWidget);
