...
class M3uHandler {
  ...
  static Future<List<M3uEntry>> parse(
    List<int> bytes, {
    ...
    try {
      ...
      throw M3uException(
        'Playlist is not valid UTF-8. Only UTF-8 M3U8 playlists are supported.',
      );
    }
    ...
    for (final line in lines) {
      ...
      if (trimmed.startsWith('#')) {
        if (trimmed.startsWith('#EXT-X-')) {
          throw M3uException(
            'HLS network playlists are not supported for local import.',
          );
  ...
  static String _resolvePath(String entryPath, String? sourceLocation) {
    ...
    final isWindows = RegExp(
      r'^[a-zA-Z]:[\\/]|^\\[\\]',
    ).hasMatch(sourceLocation);
  ...
  static M3uExportResult generate(
    List<Track> queue, {
    ...
    for (final track in queue) {
      ...
      if (exportDestinationDir != null &&
          context.isAbsolute(path) &&
          context.isAbsolute(exportDestinationDir)) {
        try {
      ...
      final safeArtist = track.artistName
          .replaceAll('\n', ' ')
          .replaceAll('\r', ' ');
