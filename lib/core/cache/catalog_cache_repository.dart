...
enum CacheSource { memory, persistent, network, staleMemory, stalePersistent }
...
class CatalogCacheRepository {
  ...
  final LinkedHashMap<String, _L1Entry<dynamic>> _l1Cache =
      LinkedHashMap<String, _L1Entry<dynamic>>();
  ...
  Stream<CacheResult<T>> watchOrFetch<T>({
    ...
    if (l1Entry != null) {
      ...
      if (age < policy.freshFor) {
        ...
        yield CacheResult(
          data: l1Entry.data as T,
          source: CacheSource.staleMemory,
          fetchedAt: l1Entry.fetchedAt,
        );
            ...
            try {
              ...
              if (age < policy.freshFor) {
                ...
                yield CacheResult(
                  ...
                  fetchedAt: driftEntry.fetchedAt,
                );
                return;
              } else {
                ...
                yield CacheResult(
                  data: decodedData,
                  source: CacheSource.stalePersistent,
                  fetchedAt: driftEntry.fetchedAt,
                );
                // Continue to network fetch in background
              }
    ...
    try {
      ...
      final fetchFuture = _inFlight.putIfAbsent(
        key,
        () => _executeFetchAndCache<T>(key, resourceType, fetch, encode),
      );
      ...
      yield CacheResult(
        ...
        fetchedAt:
            DateTime.now(), // Approximated, technically inside _executeFetchAndCache
      );
    } catch (e) {
      ...
      _inFlight.remove(
        key,
      ); // Ensure failed futures are removed immediately on error
