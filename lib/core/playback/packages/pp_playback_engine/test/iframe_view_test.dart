...
void main() {
  ...
  testWidgets('PlaybackView renders YoutubePlayer when in IFrame mode', (WidgetTester tester) async {
    // 1. Setup mock state
    ...
    when(() => mockController.statusStream).thenAnswer((_) => Stream.value(status));
  ...
  testWidgets('PlaybackView logic branch verification', (WidgetTester tester) async {
    ...
    when(() => mockController.statusStream).thenAnswer((_) => Stream.value(statusIFrame));
