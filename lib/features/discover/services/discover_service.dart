...
class DiscoverService {
  ...
  Future<DiscoverContent> _fetchConcurrentDiscoverContent(
    DiscoverSeedContext seedCtx,
  ) async {
    ...
    if (seedCtx.hasRichData) {
      ...
      if (seedCtx.favoriteTracks.isNotEmpty) {
        ...
        final favSeed =
            seedCtx.favoriteTracks[random.nextInt(
              min(5, seedCtx.favoriteTracks.length),
            )];
        futures.add(
          _spotify
      ...
      if (seedCtx.followedArtists.isNotEmpty) {
        ...
        final artistSeed =
            seedCtx.followedArtists[random.nextInt(
              min(5, seedCtx.followedArtists.length),
            )];
        futures.add(
          _spotify
  ...
  bool _isTrackId(String id, DiscoverSeedContext ctx) {
    return ctx.recentTracks.any((t) => t.spotifyId == id) ||
        ctx.favoriteTracks.any((t) => t.spotifyId == id);
  }
...
final discoverServiceProvider = Provider(
  (ref) => DiscoverService(
